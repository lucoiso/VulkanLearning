...
void RenderCore::GLFWErrorCallback(std::int32_t const Error,
...
void RenderCore::GLFWCursorPositionCallback(GLFWwindow *const Window,
                                            double const NewCursorPosX,
                                            double const NewCursorPosY)
{
    RuntimeInfo::Manager::Get().PushCallstack();
    ...
    if (g_CanMovementCamera)
    {
        ...
        Camera& Camera{
            Target->GetMutableCamera()
        };

        float const Sensitivity{
            Camera.GetSensitivity() * 0.1F
        };

        float const OffsetX{
            static_cast<float>(NewCursorPosX - LastCursorPosX) * Sensitivity
        };
        float const OffsetY{
            static_cast<float>(LastCursorPosY - NewCursorPosY) * Sensitivity
        };
...
void RenderCore::GLFWCursorScrollCallback([[maybe_unused]] GLFWwindow *const Window,
                                          [[maybe_unused]] double const OffsetX,
...
void RenderCore::InstallGLFWCallbacks(GLFWwindow *const Window,
                                      bool const InstallClose)
{
    RuntimeInfo::Manager::Get().PushCallstack();
