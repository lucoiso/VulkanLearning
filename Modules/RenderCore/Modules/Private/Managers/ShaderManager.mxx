...
bool ShaderManager::Compile(std::string_view const Source, std::vector<uint32_t>& OutSPIRVCode)
{
...
bool ShaderManager::Load(std::string_view const Source, std::vector<std::uint32_t>& OutSPIRVCode)
{
    std::filesystem::path const Path(Source);
    if (!exists(Path))
    {
...
bool ShaderManager::CompileOrLoadIfExists(std::string_view const Source, std::vector<uint32_t>& OutSPIRVCode)
{
...
VkShaderModule ShaderManager::CreateModule(VkDevice const& Device, std::vector<std::uint32_t> const& SPIRVCode, EShLanguage const Language)
{
    ...
    Helpers::CheckVulkanResult(vkCreateShaderModule(Device, &CreateInfo, nullptr, &Output));
...
VkPipelineShaderStageCreateInfo ShaderManager::GetStageInfo(VkShaderModule const& Module) const
{
    return m_StageInfos.at(Module);
}

std::vector<VkShaderModule> ShaderManager::GetShaderModules() const
{
...
std::vector<VkPipelineShaderStageCreateInfo> ShaderManager::GetStageInfos() const
{
...
void ShaderManager::FreeStagedModules(std::vector<VkPipelineShaderStageCreateInfo> const& StagedModules)
{
...
bool ShaderManager::Compile(std::string_view const Source, EShLanguage const Language, std::vector<std::uint32_t>& OutSPIRVCode)
{
...
void ShaderManager::StageInfo(VkShaderModule const& Module, EShLanguage const Language)
{
